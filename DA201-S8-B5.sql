...
SELECT AVG(total_amount) AS average_transaction_value FROM Sales;
-- Hóa đơn cao nhất
SELECT MAX(total_amount) AS highest_transaction FROM Sales;
-- Hóa đơn thấp nhất
SELECT MIN(total_amount) AS lowest_transaction FROM Sales;
-- Ngày diễn ra giao dịch đầu tiên
SELECT MIN(transaction_date) AS first_transaction_date FROM Sales;
-- Ngày diễn ra giao dịch cuối cùng
SELECT MAX(transaction_date) AS last_transaction_date FROM Sales;
